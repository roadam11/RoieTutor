...
const testimonials = [
  ...
  {
    name: "ליאת, אמא של תהל, תלמידת  כיתה י"א, מתמטיקה 5 יח"ל",
  ...
  },
  {
    ...
    text: "בוקר טוב רועי, רציתי להגיד תודה שעזרת לי למבחן במדעי המחשב 🙏 קיבלתי 94 – הציון הכי גבוה שלי עד עכשיו! (הכי גבוה שהיה לי עד היום היה 62... עכשיו ישאירו אותי במגמה 🎉)",
  },
  {
    name: "נועה, אמא של זיו, תלמיד כיתה י' מתמטיקה 4 יח"ל",
...
export default function Testimonials() {
  ...
  // Auto-slide (סטוריז) – נעצר במעבר/נגיעה
  useEffect(() => {
    if (isPaused) return
    ...
    <section id="testimonials" className="relative bg-gradient-to-b from-slate-50 to-white dark:from-ink2 dark:to-ink py-16">
      ...
      <div className="relative max-w-5xl mx-auto px-4 text-center">
        ...
        <div className="flex justify-center gap-2 mt-6">
          {testimonials.map((_, i) => (
            <button key={i} onClick={() => goTo(i)} aria-label={`המלצה ${i + 1}`}
